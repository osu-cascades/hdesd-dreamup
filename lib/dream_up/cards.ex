defmodule DreamUp.Cards do
  ...
  def start_spinner_state(game, pivot_to_remove \\ nil, last_card \\ nil) do
    ...
    case pivot_to_remove do
      "red" ->
        Games.update_game(game, %{"time_left" => ~T[00:00:01], "round_state" => "SPINNER", "red_pivot_token" => false, "red_pivoted_method_id" => last_card.id, ("method_" <> Integer.to_string(game.round_number) <> "_id") => picked_card.id})
      "blue" ->
        Games.update_game(game, %{"time_left" => ~T[00:00:01], "round_state" => "SPINNER", "blue_pivot_token" => false, "blue_pivoted_method_id" => last_card.id, ("method_" <> Integer.to_string(game.round_number) <> "_id") => picked_card.id})
      nil ->
        IO.inspect(game.round_number + 1)
        Games.update_game(game, %{"time_left" => ~T[00:00:01], "round_state" => "SPINNER", "round_number" => game.round_number + 1, ("method_" <> Integer.to_string(game.round_number + 1) <> "_id") => picked_card.id})
    end
